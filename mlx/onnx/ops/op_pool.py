...
from .pad import auto_pad as ap
from .pad import convert_pad


def compute_strides(shape: List[int]):
    return list(
        reversed(mx.cumprod(mx.array([1] + list(reversed(shape))))[:-1].tolist())
    )
...
def MaxPool(
    ...
    auto_pad="NOTSET",
    ceil_mode=0,
    dilations: Optional[mx.array] = None,
    pads=None,
    storage_order=0,
    strides=None,
):
...
def Pool(
    ...
    if ceil_mode == 1:
        x = mx.pad(
            x,
            pad_width=[(0, 0), (0, 0)] + [(0, 1)] * (x.ndim - 2),
